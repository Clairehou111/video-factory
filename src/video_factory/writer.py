...
class StoryWriterPacket:
    ...
    def prompt(self) -> str:
        ...
        if self.topic_type == TopicType.GITHUB_PROJECT:
            ...
            radar_copy_contract = (
                "Radar visible-copy contract: ordinary headlines fit at most 20 Chinese-character-equivalents; when preserving an exact subject/model/project name makes that impossible, at most 28 and never more than two rendered lines. A highlighted Chinese gloss is usually 16–24 equivalents. Evidence copy defaults to 40 equivalents for a two-second glance, but may grow to 64 only when the shot duration is explicitly extended; keep the font large and use 2–4 lines rather than shrinking type. Write like a technically literate colleague sharing one useful discovery in a group chat: everyday Chinese, an explicit subject and action, short clauses, and a concrete reason the viewer should care. Every concise narrative sentence contains at least one concrete metric, action verb, or named entity. A competitor or causal association absent from direct evidence may appear only as an explicit question and must be copied into editorial_inference; proof and takeaway must not assert it. category_label is optional source metadata but is not rendered in the video. direct_identifier must be copied exactly from supplied evidence or remain empty. "
                if radar else ""
            )
            ...
            github_contract = (
                ...
                "Never assume a named technical person is already famous to a mainland-Chinese audience. Every people_change story must spend one early evidence-backed beat establishing why the main person matters: their current/former role plus one to three recognizable systems, products, papers, or projects they built or led. This identity anchor is context, not the main story, and cannot replace the actual move. Even globally prominent people benefit from a compact role anchor; do not maintain a celebrity allowlist. "
                "Write earlier people-move context as a concrete dated event chain: name who left which organization and where they went or what they founded. Turn a reported headline into audience copy only when the cited evidence itself supplies those actions. If the available evidence contains only an abstract industry summary, choose a more specific sourced event or omit the background beat. The audience should see the evidence for a pattern, not the editor's label for it. "
                "Every price_competition story must answer two questions beyond the discount: what workload the discounted model or route is suited for, and what is known about delivery quality. Delivery quality means uptime/reliability plus response performance such as TTFT, latency, or throughput. Use exact current metrics only when cited. If route-specific metrics are absent, visibly ask whether the cheap route is stable and fast enough, then give the evidence-backed verification action (for example, inspect the route's current uptime/TTFT/throughput); never imply that a low price proves production readiness. "
                ...
                " Because an X-rooted video must show the complete root post first, the source-to-hook bridge must be obvious, but the root author does not automatically deserve first position in the hook. Lead with the most audience-recognizable evidenced entity and its concrete stake—often the incumbent company losing talent, a familiar product changing, or a vendor conflict—then name the less-familiar author/person in the same hook or immediate identity beat. For example, a Google talent-loss story should lead with Google and the loss, not with 'Jeff Dean announced'. Do not headline unrelated background over the root card; the lead entity, consequence, and root author's role must still form one evidenced event."
