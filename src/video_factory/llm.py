    ...
    def plan(self, packet: StoryWriterPacket) -> tuple[EditorialPlan, dict[str, object]]:
        ...
        messages = [
            ...
            {"role": "user", "content": "\n".join([
                ...
                "For a person/team move, use one context question/search query to establish the main person's role and recognizable work for viewers who may not know them, plus one context question/search query to test recent related moves from the same incumbent; this is how one departure gets an identity anchor and becomes an evidenced pattern or stays a single event. For a price/limit/reset update, search the author/company's preceding announcement and rollout scope. For a discounted inference route, also ask what workload it suits and whether route-specific uptime plus TTFT/latency/throughput are available; absent performance numbers become a visible verification question, not an invented claim.",
